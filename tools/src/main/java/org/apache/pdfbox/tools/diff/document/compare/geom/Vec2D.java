...
import org.apache.pdfbox.tools.diff.document.compare.geom.math.InterpolateStrategy;
import org.apache.pdfbox.tools.diff.document.compare.geom.math.MathUtils;
import org.apache.pdfbox.tools.diff.document.compare.geom.math.ScaleMap;
...
public class Vec2D implements Comparable<ReadonlyVec2D>, ReadonlyVec2D {
	...
	public static final ReadonlyVec2D NEG_MAX_VALUE = new Vec2D(-Float.MAX_VALUE, -Float.MAX_VALUE);
	...
	public Vec2D getMapped(ScaleMap map) {
		return new Vec2D((float) map.getClippedValueFor(x), (float) map.getClippedValueFor(y));
	}
	...
	public final boolean isZeroVector() {
		return MathUtils.abs(x) < MathUtils.EPS && MathUtils.abs(y) < MathUtils.EPS;
	}
